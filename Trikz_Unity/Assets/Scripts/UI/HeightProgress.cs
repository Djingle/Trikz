...
public class HeightProgress : VisualElement {
	public new class UxmlTraits : VisualElement.UxmlTraits {
		...
		public override void Init(VisualElement ve, IUxmlAttributes bag, CreationContext cc) {
			...
			(ve as HeightProgress).progress = m_ProgressAttribute.GetValueFromBag(bag, cc);
		}
	}

	// Define a factory class to expose this control to UXML.
	public new class UxmlFactory : UxmlFactory<HeightProgress, UxmlTraits> { }
	...
	// This default constructor is HeightProgress's only constructor.
	public HeightProgress() {
	...
	static void CustomStylesResolved(CustomStyleResolvedEvent evt) {
		HeightProgress element = (HeightProgress)evt.currentTarget;
	...
	public void GenerateVisualContent(MeshGenerationContext context) {
		...
		painter.BeginPath();
		painter.Arc(new Vector2(width * 0.5f, height * 0.5f), width * 0.5f, -180f, 0f);
		painter.Stroke();

		// Draw the progress
		painter.strokeColor = m_ProgressColor;
		painter.BeginPath();
		painter.Arc(new Vector2(width * 0.5f, height * 0.5f), width * 0.5f, 0f, -360.0f * (progress / 2f), ArcDirection.CounterClockwise);
		painter.Stroke();
	}
}